...
defined('BASEPATH') OR exit('No direct script access allowed');
    if(!function_exists("read_csv_data")){
        function read_csv_data($nom_fichier, $ligneDeb = 1, $ligneFin = -1)
        {
            ...
            if (($fichier_handle = fopen($fichier, 'r')) !== FALSE) {
                $nligne = 1;
                while (($ligne = fgetcsv($fichier_handle, 1000, ',')) !== FALSE) {
                    if($ligneFin > 0) {
    ...
    if(!function_exists("setup_docs")){
        function setup_docs(){
            ...
            $config['allowed_types']='jpeg|jpg|png|pdf|csv|JPG|PNG';
            $ci=& get_instance();
            $ci->upload->initialize($config);
        }
    }
